...
# Boots the FAT12 floppy with the FAT32 hard disk attached, then presses 'a' so
# the kernel loads HELLOAPP.LEO from FAT32, validates the LEO1 header, copies it
# into a kernel page, and calls its entry cooperatively in ring 0. The app
# proves it ran by emitting serial text through the in-kernel API table.
...
function Start-LeonOs32AppTest {
...
$Output = Start-LeonOs32AppTest "LeonOS-32BitHddApp" @(
    "sendkey a",
    "sleep 1500"
)
...
foreach ($Bad in @(
    "LEO1 app not found", "LEO1 app bad", "LEO1 app read failed",
    "LEO1 app too large", "LEO1 app no allocator")) {
    if ($Output -like "*$Bad*") {
        throw "Kernel reported a LEO1 loader failure: $Bad"
    }
}

Write-Host "QEMU 32-bit LEO1 app loader test passed."
