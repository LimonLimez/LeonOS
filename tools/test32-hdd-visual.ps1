...
$Suffix = $Resolution
$PpmPath = Join-Path $OutputRoot "leonos32-hdd-visual-test-$Suffix.ppm"
$PngPath = Join-Path $OutputRoot "leonos32-hdd-visual-test-$Suffix-scaled.png"
...
$StartInfo.Arguments = "-name LeonOS-32BitHddVisual-$Suffix -machine pc -cpu qemu32 -m 32M -drive file=`"$EscapedImagePath`",format=raw,if=floppy -drive file=`"$EscapedHddPath`",format=raw,if=ide,index=0,media=disk -boot a -vga std -display none -serial none -monitor tcp:127.0.0.1:$MonitorPort,server,nowait -no-reboot"
...
try {
    ...
    if ($Stderr.Trim().Length -gt 0) {
        Write-Host "QEMU stderr (LeonOS-32BitHddVisual-$Suffix):"
        Write-Host $Stderr
    }
} finally {
    if ($Client) {
...
Write-Host "QEMU 32-bit FAT32 HDD visual test passed ($Resolution / $($Size.Width)x$($Size.Height))."
