...
static void qjs_install_browser_bootstrap(JSContext *ctx)
{
	static const char bootstrap[] =
		...
		"function domArg(v){return v&&typeof v==='object'&&v.nodeType?v:textNode(v);}"
		"function firstNode(n){return n&&(n.firstChild||(n.childNodes&&n.childNodes[0])||null);}"
		"function insertMany(p,ref,args){if(!p||!p.insertBefore)return;for(var i=0;i<args.length;i++)p.insertBefore(domArg(args[i]),ref||null);}"
		"function clearKids(p){var guard=0;while(p&&p.removeChild&&firstNode(p)&&guard++<2048)p.removeChild(firstNode(p));"
		"if(p&&p.childNodes&&!firstNode(p)){p.childNodes.length=0;p.children=p.childNodes;resyncKids(p);}}"
		"function removeSelf(n){if(n&&n.parentNode&&n.parentNode.removeChild)n.parentNode.removeChild(n);}"
		"function parentBefore(n,args){var p=n&&n.parentNode;if(!p||!p.insertBefore)return;for(var i=0;i<args.length;i++)p.insertBefore(domArg(args[i]),n);}"
		"function parentAfter(n,args){var p=n&&n.parentNode,ref=n&&n.nextSibling;if(!p||!p.insertBefore)return;for(var i=0;i<args.length;i++)p.insertBefore(domArg(args[i]),ref||null);}"
		"var parentProtos=[Element.prototype,Document.prototype,DocumentFragment.prototype];"
		"for(var ppi=0;ppi<parentProtos.length;ppi++){var pp=parentProtos[ppi];"
		"pp.append=pp.append||function(){insertMany(this,null,arguments);};"
		"pp.prepend=pp.prepend||function(){insertMany(this,firstNode(this),arguments);};"
		"pp.replaceChildren=pp.replaceChildren||function(){clearKids(this);insertMany(this,null,arguments);};}"
		"Node.prototype.before=Node.prototype.before||function(){parentBefore(this,arguments);};"
		"Node.prototype.after=Node.prototype.after||function(){parentAfter(this,arguments);};"
		"Node.prototype.remove=Node.prototype.remove||function(){removeSelf(this);};"
		"Node.prototype.replaceWith=Node.prototype.replaceWith||function(){parentBefore(this,arguments);removeSelf(this);};"
		"function adjacent(n,pos,args){pos=String(pos||'').toLowerCase();"
		"if(pos==='beforebegin')return parentBefore(n,args);if(pos==='afterend')return parentAfter(n,args);"
		"if(pos==='afterbegin')return n&&n.prepend&&n.prepend.apply(n,args);"
		"if(pos==='beforeend')return n&&n.append&&n.append.apply(n,args);}"
		"Element.prototype.insertAdjacentElement=Element.prototype.insertAdjacentElement||function(pos,el){adjacent(this,pos,[el]);return el;};"
		"Element.prototype.insertAdjacentText=Element.prototype.insertAdjacentText||function(pos,text){adjacent(this,pos,[textNode(text)]);};"
		"Element.prototype.insertAdjacentHTML=Element.prototype.insertAdjacentHTML||function(pos,html){"
		"var d=this.ownerDocument||g.document,tmp=d&&d.createElement?d.createElement('div'):el('div');"
		"tmp.innerHTML=String(html||'');var kids=tmp.childNodes||[];for(var i=0;i<kids.length;i++)adjacent(this,pos,[kids[i]]);};"
...
static void qjs_maybe_run_selector_selftest(JSContext *ctx)
{
	...
	static const char selftest[] =
		...
		"var mix=document.createElement('section'),before=document.createElement('i'),"
		"target=document.createElement('b'),after=document.createElement('u'),"
		"repl=document.createElement('small'),doomed=document.createElement('del'),"
		"wrap=document.createElement('div'),wrapChild=document.createElement('span'),"
		"adjEl=document.createElement('q');"
		"before.className='mix-before';target.id='mix-target';after.className='mix-after';"
		"repl.className='mix-repl';doomed.id='mix-doomed';wrapChild.className='replace-child';adjEl.className='adj-after';"
		"mix.append(target);target.before(before);target.after(after);after.replaceWith(repl);"
		"mix.prepend('start');mix.append(doomed);doomed.remove();wrap.append('old');"
		"wrap.replaceChildren(wrapChild,'new');root.append(mix,wrap);"
		"target.insertAdjacentHTML('beforebegin','<mark class=\"adj-before\">A</mark>');"
		"target.insertAdjacentElement('afterend',adjEl);wrap.insertAdjacentText('beforeend','tail');"
		...
		"var methodOk=typeof mix.append==='function'&&typeof target.before==='function'&&"
		"typeof target.insertAdjacentHTML==='function'&&typeof target.insertAdjacentElement==='function'&&"
		"typeof wrap.replaceChildren==='function'&&mix.querySelectorAll('i.mix-before + mark.adj-before').length===1&&"
		"mix.querySelectorAll('mark.adj-before + b#mix-target').length===1&&"
		"mix.querySelectorAll('b#mix-target + q.adj-after').length===1&&"
		"mix.querySelectorAll('#mix-doomed').length===0&&wrap.querySelectorAll('span.replace-child').length===1;"
